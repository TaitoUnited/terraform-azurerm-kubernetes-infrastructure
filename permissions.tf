...
resource "random_uuid" "owner_kubeadmin_id" {
  count                = var.kubernetes_name != "" ? length(var.owners) : 0
  keepers = {
    acr_id  = azurerm_container_registry.acr.id
    sp_id   = var.owners[count.index]
...
resource "random_uuid" "developer_kubeuser_id" {
  count                = var.kubernetes_name != "" ? length(var.developers) : 0
  keepers = {
    acr_id  = azurerm_container_registry.acr.id
    sp_id   = var.developers[count.index]
...
resource "azurerm_role_assignment" "developer_kubeuser" {
  ...
  principal_id         = var.developers[count.index]
